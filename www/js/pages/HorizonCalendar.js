                            ...
                            <div style="flex: 1;">
                                ...
                                <div class="details-section" style="display: ${isExpanded ? 'block' : 'none'}; margin-top: 12px; animation: fadeIn 0.3s ease;">
                                    ...
                                    <div style="background: rgba(255,255,255,0.05); padding: 12px; border-radius: var(--radius-sm); margin-bottom: 12px; display: flex; align-items: center; gap: 12px;">
                                        <div style="font-size: 2rem;">${moon.icon}</div>
                                        <div>
...
function renderCalendar(date, container, events, onNav, onDayClick) {
    ...
    container.innerHTML = `
        <div class="flex justify-between items-center" style="margin-bottom: var(--spacing-md);">
            <button id="prev-month" class="btn btn-secondary" style="padding: 8px 16px;">&lt;</button>
            <h3 style="margin: 0; font-size: 1.25rem;">${monthNames[month]} ${year}</h3>
            <button id="next-month" class="btn btn-secondary" style="padding: 8px 16px;">&gt;</button>
        </div>
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        if (dayEvents.length > 0) {
            // Container for event indicators (subtle bars at the bottom)
            const indicatorContainer = document.createElement('div');
            indicatorContainer.style.display = 'flex';
            indicatorContainer.style.gap = '2px';
            indicatorContainer.style.marginTop = 'auto';
            indicatorContainer.style.padding = '0 2px';

            dayEvents.slice(0, 3).forEach(evt => {
                const bar = document.createElement('div');
                bar.className = 'calendar-event-pill';
                // Use a default accent color for all indicators for visual consistency
                bar.style.backgroundColor = 'var(--color-accent)';
                indicatorContainer.appendChild(bar);
            });
            dayBtn.appendChild(indicatorContainer);
            dayBtn.style.borderColor = 'rgba(52, 152, 219, 0.4)';
        } else {
            // Only show moon phase icon if NO events are present
            const moonDiv = document.createElement('div');
            moonDiv.className = 'calendar-moon-overlay';
            moonDiv.innerHTML = moon.icon;
